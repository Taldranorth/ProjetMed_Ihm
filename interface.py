...
def statetax(gamedata, classmap, option):
	if gamedata.changenewstate("tax") == True:
		...
		for village in player.fief:
			...
			###### BLOCAGE au niveau de l'affichage de la taxe et ne centre plus, a voir comment faire plus prore aussi ######
			lc_interface_tax.insert(tkinter.END, f"{village.name}: {taxes} écus")

		lc_interface_tax.bind("<Button-1>", lambda event: centervillage(event, gamedata, classmap, option))
